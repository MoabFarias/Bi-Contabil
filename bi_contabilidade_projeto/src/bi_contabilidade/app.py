...
def validar_periodo(periodo: str | None, padrao: str) -> str:
    if not periodo:
        return padrao
    periodo = periodo.strip()
    if len(periodo) == 7 and periodo[4] == "-" and periodo[:4].isdigit() and periodo[5:].isdigit():
        return periodo
    return padrao
...
def dashboard(request: Request):
    ...
    validacao = ler_json(LOGS_DIR / "validacao_modelo.json", {"status": "sem_validacao", "mensagem": "Nenhuma validacao executada."})
    balancete = ler_json(LOGS_DIR / "balancete_resumo.json", {"status": "sem_balancete", "mensagem": "Nenhum balancete gerado."})
    dre_resumo = ler_json(LOGS_DIR / "dre_resumo.json", {"status": "sem_dre", "mensagem": "Nenhuma DRE gerada."})
    dre_detalhe = ler_json(LOGS_DIR / "dre_detalhe.json", {"grupos": []})
    dre_comparativo = ler_json(LOGS_DIR / "dre_comparativo.json", {"status": "sem_comparativo", "linhas": [], "periodo_base": None, "periodo_comparacao": None, "periodos_disponiveis": []})
...
def gerar_dre_comparativo(periodo_base: str = Form("2026-01"), periodo_comparacao: str = Form("2025-01")):
    base = validar_periodo(periodo_base, "2026-01")
    comparacao = validar_periodo(periodo_comparacao, "2025-01")
    executar_script("motor_dre_comparativo.py", "--base", base, "--comparacao", comparacao)
    return RedirectResponse(url="/", status_code=303)
...
def download_balancete():
    ...
    return FileResponse(path=arquivo, filename="balancete_mensal.xlsx", media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
...
def download_dre():
    ...
    return FileResponse(path=arquivo, filename="dre_gerencial.xlsx", media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
...
def download_dre_comparativo():
    ...
    return FileResponse(path=arquivo, filename="dre_comparativo.xlsx", media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
